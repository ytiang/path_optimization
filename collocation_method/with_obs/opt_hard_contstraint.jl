...
qf = (40.0, 40.0, -1.2) # final configuration
N = 9
...
@NLexpression(mdl, sₖ[i=1:N], sf/2*(node[i]+1))
@NLexpression(mdl, sₖᵢ[i=1:N*N], sₖ[Int(ceil(i/9))]/2*(node[index(i)] + 1))
@NLexpression(mdl, exprx[i=1:N*N],
    cos(q0[3] + k0*sₖᵢ[i] + b/2*sₖᵢ[i]^2 + c/3*sₖᵢ[i]^3 + d/4*sₖᵢ[i]^4 + e/5*sₖᵢ[i]^5 + f/6*sₖᵢ[i]^6)*sₖ[Int(ceil(i/9))]/2)
@NLexpression(mdl, expry[i=1:N*N],
    sin(q0[3] + k0*sₖᵢ[i] + b/2*sₖᵢ[i]^2 + c/3*sₖᵢ[i]^3 + d/4*sₖᵢ[i]^4 + e/5*sₖᵢ[i]^5 + f/6*sₖᵢ[i]^6)*sₖ[Int(ceil(i/9))]/2)
@NLexpression(mdl, xₖ[i=1:N], q0[1]+sum(exprx[(i-1)*9+j]*coe[j] for j=1:N))
@NLexpression(mdl, yₖ[i=1:N], q0[2]+sum(expry[(i-1)*9+j]*coe[j] for j=1:N))
x_obs = 15.0
y_obs = 10.0
r = 5.0
@NLexpression(mdl, obs[i=1:N], exp(-((xₖ[i]-x_obs)^2+(yₖ[i]-y_obs)^2-r^2 )) )

@NLexpression(mdl, expr_x[i=1:N],
    cos(q0[3] + k0*sₖ[i] + b/2*sₖ[i]^2 + c/3*sₖ[i]^3 + d/4*sₖ[i]^4 + e/5*sₖ[i]^5 + f/6*sₖ[i]^6)*sf/2)
@NLexpression(mdl, expr_y[i=1:N],
    sin(q0[3] + k0*sₖ[i] + b/2*sₖ[i]^2 + c/3*sₖ[i]^3 + d/4*sₖ[i]^4 + e/5*sₖ[i]^5 + f/6*sₖ[i]^6)*sf/2)
...
rx[:plot](X, Y)
θo = -3.1:0.1:3.14
xo = x_obs*ones(length(θo)) + r*cos(θo)
yo = y_obs*ones(length(θo)) + r*sin(θo)
rx[:plot](xo, yo)
