...
N = 5 # number of curve segment, (N+1) collocation points
...
@NLobjective(mdl,
    Min,
    0.01*tf + sum((tk[i+1]-tk[i])/2*sum(u[degree*(i-1)+j]^2*lagrange_pt[2][j] for j=1:degree) for i=1:N))
